...
def get_point_info(points, mask_ratio=0, mask=-1):
  with tf.name_scope('points_info'):
    ...
    # mask the index 
    # use the index (32, ?, 3) Open3d, trimesh. 
...
class ComputeGraphSeg:
  ...
  def __call__(self, dataset='train', training=True, reuse=False, gpu_num=1):
    ...
    for i in range(gpu_num):
      with tf.device('/gpu:%d' % i):
        with tf.name_scope('device_%d' % i):
          octree, _, points = data_iter.get_next()  # points: (32,) tensor of strings (unknown encoding)
          # print('octree:', octree.get_shape()) 
          ...
          logit, backbone_feature, fc_1 = seg_network(octree, FLAGS.MODEL, training, reuse, pts=pts)
          ...
          if flags_data.batch_size == 1:
            print('flags_data.batch_size:', flags_data.batch_size)
    ...
    return tensors, names, pts, fc_1, label, logit
