...
def ssa_layer(conv, index):
  # print('attention calculating')
  q, attn = multihead_attention(conv, conv, conv, index, 1, 256, 256, 256)  
  return q

def predict_module(data, num_output, num_hidden, training, index=None, ssa=False):
  # MLP with one hidden layer
  with tf.variable_scope('conv1'):
    conv = octree_conv1x1_bn_relu(data, num_hidden, training)
    # print('conv:', conv.shape, conv.get_shape()) # [1, 256, ?, 1]
  # if ssa:
  #   with tf.variable_scope('ssa-layer'):
  #     conv = ssa_layer(conv, index)
  with tf.variable_scope('conv2'):
    logit = octree_conv1x1(conv, num_output, use_bias=True)
    # print('logit:', logit.shape, logit.get_shape()) #  [1, 15, ?, 1]
    # sys.exit()
  return logit, conv
