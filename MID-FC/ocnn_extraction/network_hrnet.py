...
class HRNet:
  ...
  def network_seg(self, octree, training, reuse=False, pts=None, mask=None):
    with tf.variable_scope('ocnn_hrnet', reuse=reuse):
      ...
      # print('backbone conv:', convs[0].get_shape())
      ...
      with tf.variable_scope('seg_header'): 
        if pts is None:
          ...
          logit, feature, fc_1 = self.seg_header_pts(convs, octree, self.flags.nout, pts, training)
          # print('network_seg: getting feature')
        self.tensors['logit_seg'] = logit

    return logit, feature, fc_1
  ...
  def seg_header(self, inputs, octree, nout, mask, training):
    ...
    with tf.variable_scope('predict_%d' % depth_out):
      logit = predict_module(feature, nout, 128 * factor, training, ssa=False) # 2-FC
  ...
  def seg_header_pts(self, inputs, octree, nout, pts, training):
    ...
    points_features = feature #  (1, 896, 100792, 1)
    
    depth_out, factor = self.flags.depth_out, self.flags.factor
    xyz, ids = tf.split(pts, [3, 1], axis=1) # original code
    index_ids = ids
    ...
    if depth_out == 6:
      ...
      feature = tf.concat([feature, conv6], axis=1) #  (1, 928, 154551, 1)

    with tf.variable_scope('predict_%d' % depth_out):
      logit, fc_1 = predict_module(feature, nout, 128 * factor, training, index=index_ids, ssa=False) # 2-FC
      logit = tf.transpose(tf.squeeze(logit, [0, 3])) # (1, C, H, 1) -> (H, C)  (154551, 15)
