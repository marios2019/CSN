...
from torch.optim.lr_scheduler import LambdaLR, StepLR, ReduceLROnPlateau, ExponentialLR
...
def initialize_scheduler(optimizer, config, last_step=-1, factor=0.5, patience=10, cooldown=10, verbose=False):
	if config.scheduler == 'StepLR':
		...
		return ReduceLROnPlateau(optimizer, patience=patience, cooldown=cooldown, factor=factor, verbose=verbose)
	else:
		logging.error('Scheduler not supported')


def initialize_backbone_scheduler(optimizer, config, last_step=-1, factor=0.5, patience=10, cooldown=10, verbose=False):
	if config.backbone_scheduler == 'StepLR':
		return StepLR(
			optimizer, step_size=config.step_size, gamma=config.step_gamma, last_epoch=last_step)
	elif config.backbone_scheduler == 'PolyLR':
		return PolyLR(optimizer, max_iter=config.max_iter, power=config.poly_power, last_step=last_step)
	elif config.backbone_scheduler == 'SquaredLR':
		return SquaredLR(optimizer, max_iter=config.max_iter, last_step=last_step)
	elif config.backbone_scheduler == 'ExpLR':
		return ExponentialLR(optimizer, config.exp_gamma, verbose=verbose)
	elif config.backbone_scheduler == 'ReduceLROnPlateau':
		return ReduceLROnPlateau(optimizer, patience=patience, cooldown=cooldown, factor=factor, verbose=verbose)
	else:
		logging.error('Scheduler not supported')


def initialize_header_scheduler(optimizer, config, last_step=-1, factor=0.5, patience=10, cooldown=10, verbose=False):
	if config.header_scheduler == 'StepLR':
		return StepLR(
			optimizer, step_size=config.step_size, gamma=config.step_gamma, last_epoch=last_step)
	elif config.header_scheduler == 'PolyLR':
		return PolyLR(optimizer, max_iter=config.max_iter, power=config.poly_power, last_step=last_step)
	elif config.header_scheduler == 'SquaredLR':
		return SquaredLR(optimizer, max_iter=config.max_iter, last_step=last_step)
	elif config.header_scheduler == 'ExpLR':
		return ExpLR(
			optimizer, step_size=config.exp_step_size, gamma=config.exp_gamma, last_step=last_step)
	elif config.header_scheduler == 'ReduceLROnPlateau':
		return ReduceLROnPlateau(optimizer, patience=patience, cooldown=cooldown, factor=factor, verbose=verbose)
	else:
		logging.error('Scheduler not supported')
